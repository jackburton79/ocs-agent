...
{
	try {
//	        if (!CommandExists("rpm"))   return false;
                popen_streambuf rpms("rpm -qai", "r");
		std::istream iStreamRpms(&rpms);
		std::string string;
		std::string strtmp;
//		std::multimap<std::string, std::string> softwareInfo;
		std::string comments = "";
		std::string size = "";
		std::string from = "";
...
//			if (string=="") continue;
			...
			//std::cout << "new software : " << name <<" | "<< from  <<" | "<< size <<" | "<< installdate<<" | " << version<<" | "<< comments <<"\n";
				}
			//then update name
			name=string.substr(13,35); trim(name);
			}
			else if (string.find("Version     :") != std::string::npos){
				version=string.substr(13,35); trim(version);}
			else if (string.find("Architecture:") != std::string::npos){
				strtmp = string.substr(13,35);trim(strtmp); name=name+'.'+strtmp;}
			else if (string.find("Release     :") != std::string::npos){
				strtmp = string.substr(13,35);trim(strtmp);version=version+'-'+strtmp;}
			else if (string.find("Install Date:") != std::string::npos){
				installdate=string.substr(13,50);trim(installdate);}
			else if (string.find("Size        :") != std::string::npos){
				size=string.substr(13,35);trim(size);}
			else if (string.find("Summary     :") != std::string::npos){
				comments=string.substr(13,100);trim(comments);}
			else if (string.find("Vendor      :") != std::string::npos){
				strtmp = string.substr(13,35);trim(strtmp);from="RPM-"+ strtmp;}
